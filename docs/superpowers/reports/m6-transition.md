...
| 키 입력 → 다음 프레임 p95 (5k줄 TS, 하이라이팅 ON) | < 16 ms | **9.7 ms** (max 14.3) | 숨김 창은 `backgroundThrottling`으로 rAF가 지연됨 → `MORU_HIDDEN`일 때 끔. GitHub 러너(GPU 없음)는 macOS 36 ms / Windows 830 ms로 프레임 측정이 무의미 → CI에서는 기록만, 단언은 실제 머신에서 |
